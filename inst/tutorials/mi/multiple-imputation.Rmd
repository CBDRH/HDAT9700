---
title: "HDAT9700: Missing Data and Multiple Imputation"
tutorial: 
  id: "au.edu.unsw.cbdrh.hdat9700.tutorials.missing.data.and.multiple.imputation"
output: learnr::tutorial
runtime: shiny_prerendered
css: css/tutorials.css
description: "Learn how to describe patterns of missing data, distinguish between different missing data mechanisms and implement multiple imputation of missing data in R"
---
...
3. Implement multiple imputation, a leading approach to rigorously address missing data 

### Pre-reading
The pre-reading for this chapter is [Multiple imputation for missing data in epidemiological and clinical research: potential and pitfalls, Jonathan AC Sterne, Ian R White, John B Carlin, Michael Spratt, Patrick Royston, Michael G Kenward, Angela M Wood, James R Carpenter BMJ 2009](https://media.openlearning.com/HauTHqAeFL6gQ2yxK53TQY8YnM7TcngEGnHoXa42JP9GXr8RoSjAeftMXCwWFwpv.1539904807/Sterne_2009_-_Multiple_imputation_for_missing_data_potentials_and_pitfalls.pdf) 

### Core reading 
The reference text for this chapter is "Flexible Imputation of Missing Data" by Stef Van Buuren, which is available online [here](https://stefvanbuuren.name/fimd/). This is an excellent reference book, but reading an entire book is clearly beyond the scope of a single chapter of this course. Key topics have been extracted and simplified in these notes but please refer to the textbook for a more rigorous treatment as required. 
...
**Issue 1** _can_ be ameliorated by collecting more data. This is not always an option of course, and too much missing data can lead to less precise estimates.
...
What direction is the systematic missing data bias in this example? Would the average income based on the observed data be higher or lower that the true value in the full sample?

</div>



## Illustration {data-progressive=TRUE}
...
1. The correlation between the true value of height and the probability of height being unobserved. 
2. The proportion of missing values.

**Try it out below!**

Using the app can you demonstrate the following three points?

1. If the correlation between the true value of height and the probability of height being unobserved is zero, then the estimate of height based on the observed data is unbiased **regardless of the proportion of missing data**. 

2. As the magnitude of the correlation increases, the bias increases (i.e. the gap between the known sample mean and the observed data estimate of the mean).

3. A negative correlation results in a positive bias and a positive correlation results in a negative bias.

_You can also run this app directly from the R Console by entering_ `hdat9700tutorials::run("missing-data-1")`
...
fluidPage(
    ...
    column(width = 7,
           br(),
           helpText("A positive correlation between height and the chances of height being observed means that tall individuals are more likely to have missing data. As a result the estimate for average height will be biased downwards (lower than the known sample value)."),
           br(),
           helpText("A negative correlation between height and the chances of height being observed means that short individuals are more likely to have missing data. As a result the estimate for average height will be biased upwards (higher than the known sample value).")
           )
)
```
...
# Impute data based on user input
    test <- reactive({
        mice::ampute(dt,
                     prop = input$pct,
                     patterns = c(0,1),
    ...
    # Create a dataframe indicating the missing values of height
    df <- reactive({
  ...
  # Calculate summary measures to pass to value boxes  
  bias <- reactive({
...
quiz(caption = "Quiz 2: Missing Data Mechanisms",
  question("In a study on antidepressants, patients with higher scores on a baseline depression and anxiety scale were less likely to return a follow up questionnaire",
...
```{r exercise1_2, exercise.cap = "Exercise 1.2", exercise=TRUE, exercise.eval=FALSE, exercise.setup = "exercise1_1", exercise.lines=8}
...
```{r exercise1_3, exercise.cap = "Exercise 1.3",  exercise=TRUE, exercise.eval=FALSE, exercise.setup = "exercise1_1", exercise.lines=14, fig.width=7, fig.height=5}
aggr(sleep)
...
```{r exercise1_4, exercise.cap = "Exercise 1.4",  exercise=TRUE, exercise.eval=FALSE, exercise.setup = "exercise1_1", exercise.lines=6}
...
```{r exercise1_5, exercise.cap = "Exercise 1.5",  exercise=TRUE, exercise.eval=FALSE, exercise.setup = "exercise1_1", exercise.lines=20, fig.width=7, fig.height=7}
# This figure shows that most missing values of nonD occurred for values of BodyWgt <1000
...
```{r exercise1_6, exercise.cap = "Exercise 1.6",  exercise=TRUE, exercise.eval=FALSE, exercise.setup = "exercise1_1", exercise.lines=8, fig.width=7, fig.height=7}
...
**Figure 5** Stochastic regression imputation *(Adapted from Van Buuren, Ch1.)*
...
```{r exercise2_2, exercise.cap = "Exercise 2.2", exercise=TRUE, exercise.setup = "exercise2_1", exercise.eval=FALSE, exercise.lines=5}
...
```{r exercise2_3, exercise.cap = "Exercise 2.3", exercise=TRUE, exercise.setup = "exercise2_1", exercise.eval=FALSE, exercise.lines=10}
...
# sleep$NonD.mean <- replace(sleep$NonD, is.na(sleep$NonD), XXX)
```

```{r exercise2_3-hint-2, exercise.eval=FALSE}
sleep$NonD.mean <- replace(sleep$NonD, is.na(sleep$NonD), mean(sleep$NonD,na.rm=TRUE))
```
...
```{r exercise2_4, exercise.cap = "Exercise 2.4", exercise=TRUE, exercise.setup = "exercise2_3", exercise.eval=FALSE, exercise.lines=10}
...
When data are missing on one variable only, it is possible to draw imputations using a univariate method, however things are more complex when multiple variables are incomplete (which is typically the case).
...
![Iterative Chained Equations (ICE)](https://youtu.be/VSEqd6MuKP0)
...
![Advantages of the ICE algorithm](https://youtu.be/H1kAcipjUH4)
...
![How the ICE algorithm works](https://youtu.be/XGy-OYqvC-0)
...
![How many cycles for ICE?](https://youtu.be/QxLMUv7YKU0)
...
Now lets impute again, specifying our new personalised method vector with the `method=` option. The `quad.outcome` argument indicates the variable that has a quadratic relationsip with **NonD**, i.e. **Gest**.

```{r demo8, exercise=FALSE, echo=TRUE, results='hide', warning=FALSE}
imp2 <- mice(sleep, method=meth2, quad.outcome = 'Gest', seed=8102)
```
...
When imputing such variables we would like to preserve the relationship between height, weight and BMI. If all three variables were imputed, the resulting values of BMI would not necessarily equal weight divided by height squared. To maintain the desired relationship, we can first impute values of the underlying variables (height and weight) and then use these imputed variables to derive BMI. This is called _passive imputation_.
...
NonD=Sleep-Dream
$$
We can see in the previous imputations that this is not necessarily the case when we examine the first few rows of the first imputed dataset:
...
### Exercise 3.1 (1 of 8) Familiarise yourself with the NHANES data. What is the pattern of missing values? Which variables are incomplete? What proportion of the dataset is fully observed?
...
### Exercise 3.2 (2 of 8) How does missingness vary by age group?
...
### Exercise 3.3 (3 of 8) Impute missing values in NHANES using multiple imputation. Examine the traceplot to check for chain convergence.
...
### Exercise 3.4 (4 of 8) Explore the chain convergence when you increase the chain length (i.e. the number of iterations)
...
### Exercise 3.5 (5 of 8) Explore the imputed values. Are there any unusual imputations?
...
### Exercise 3.6 (6 of 8) Use passive imputation to impute the variable bmi-squared
...
# Rerun the imputation with the customised methods vector and predictor matrix
imp4 <- mice(nhanes2, method=meth, predictorMatrix=pred, print=FALSE)
summary(imp4)
```

### Exercise 3.7 (7 of 8) Fit the model of interest in each imputed dataset
...
### Exercise 3.8 (8 of 8) Pool the model estimates. How does the MI model compare to the complete cases model?
