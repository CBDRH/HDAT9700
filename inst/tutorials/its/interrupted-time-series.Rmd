...
description: "Learn how to implement Interupted Time Series analysis to study the impact of large scale interventions using time series data."
---
...
UKDD.time <- seq(1,length(UKDD)) %>% ts(start=c(1975,1), frequency=12)
belt.after <- append(rep(0,sum(as.yearmon(time(UKDD))<'Feb 1983')),seq(1,sum(as.yearmon(time(UKDD))>='Feb 1983'))) %>% ts(start=c(1975,1), frequency=12)
belt.lag <- ifelse(as.yearmon(time(UKDD))>='Mar 1983',1,0)  %>% ts(start=c(1975,01), frequency=12)
belt.after.lag <- append(rep(0,sum(as.yearmon(time(UKDD))<'Mar 1983')),seq(1,sum(as.yearmon(time(UKDD))>='Mar 1983'))) %>% ts(start=c(1975,1), frequency=12)
...
smallpox <- read.csv(file="data/smallpox.csv", header=TRUE) 
sp <- ts(smallpox[,2], start=1750, frequency=1)
...
sp.model2 <- sarima(log(sp), p=1, d=0, q=0, xreg=cbind(sp.time, sp.vax, sp.after))

quetiapine <- read.csv(file="data/quetiapine.csv", header=TRUE)
quet <- ts(quetiapine$dispensings, start=c(2011,1),frequency=12)
...
quet.model1 <- sarima(quet, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp))

alprazolam <- read.csv(file="data/alprazolam.csv", header=TRUE)
alp <- ts(alprazolam, start=c(2014,7), end=c(2018,1), frequency=12)
step <- append(rep(0,sum(as.yearmon(time(alp))<'Feb 2017')),rep(1,sum(as.yearmon(time(alp))>='Feb 2017')))
time <- seq(from=1, to=length(alp))
after <- append(rep(0,sum(as.yearmon(time(alp))<'Feb 2017')), seq(1,sum(as.yearmon(time(alp))>='Feb 2017')))
step.lag1 <- append(rep(0,sum(as.yearmon(time(alp))<'Mar 2017')),rep(1,sum(as.yearmon(time(alp))>='Mar 2017')))
after.lag1 <- append(rep(0,sum(as.yearmon(time(alp))<'Mar 2017')), seq(1,sum(as.yearmon(time(alp))>='Mar 2017')))
month <- seasonaldummy(alp)
model1 <- lm(alp ~ time + month + step + after)
model2 <- lm(alp ~ time + month + step.lag1 + after.lag1)
```
...
2. How to set up your data for interrupted time series analysis
...
* Bernal JL, Cummins S, Gasparrini A. Interrupted time series regression for the evaluation of public health interventions: a tutorial. *Int J Epidemiol* 2016;46:348-355. [doi: 10.1093/ije/dyw098](https://academic.oup.com/ije/article/46/1/348/2622842)
...
![**Figure.** Rates of antidepressant use, psychotropic drug poisonings, and completed suicides per quarter before and after the warnings among adolescents (Source: Lu et al. 2014 *BMJ*)](images/Lu_BMJ_2014.jpg)
...
![**Figure.** Examples of potential before and after scenarios. (Modified from Soumerai et al. 2015. *Prev Chronic Dis*)](images/before_after1.png)
...
![**Figure.** Examples of potential before and after scenarios. (Modified from Soumerai et al. 2015. *Prev Chronic Dis*)](images/before_after2.png)
...
![**Figure.** Alprazolam prescribing in Australia (Schaffer et al. 2016. *JAMA Intern Med*)](images/schaffer_alprazolam.png)
...
![**Figure.** Example of negative control series. (Soumerai et al. 2015. *Prev Chronic Dis*)](images/soumerai_ctrl.png)
...
![**Figure.** Examples of intervention impacts](images/types of impacts.png)
...
b) What outcome measures would you suggest using? Think about what data are available in Australia and try to come up with multiple potential outcomes.


c) For each of these outcomes would you expect the impact to be abrupt or gradual? Permanent or temporary?
...
The time variable ($T_t$) is a sequence from 1 to $n$.
$$T_t=  1,2,3,...n $$
...
The time since the intervention ($R_t$) is defined as 0 before the intervention and increases by 1 after:

$$ R_t =  \begin{cases} 0, & \text{if} \;t<T_I \\1,2,3,...n, & \text{if}\;t \geq T_I \end{cases} $$
...
```{r UKDriverDeaths, fig.height=4, fig.width=5, exercise=TRUE, exercise.eval=FALSE, include=TRUE, echo=TRUE, tidy=TRUE}
# Load the driver deaths data
data(UKDriverDeaths)

# Plot the data
par(cex.lab=.9,cex.axis=.9,cex.main=.9)
plot(UKDriverDeaths, ylim=c(500,3000), ylab="Driver deaths", xlab="Month",main="Figure. Driver deaths in the UK (1969-1984)")
...
The first question is whether this series can be modelled using a linear regression. In the plot above, we see that the trend is mostly linear. And while there is seasonality, it appears to be predictable, and likely monthly. Thus, it may be a good option for linear regression. 
...
```{r UKDD, eval=FALSE, fig.height=4, fig.width=5, exercise=TRUE, exercise.eval=FALSE, tidy=TRUE,  dependson="UKdeaths"}
...
It is important to only consider lagged or delayed effects when it makes sense from a practical or clinical perspective - given what you know about the intervention and outcome, is it possible that 
```{r UKDD3, exercise=TRUE, exercise.eval=FALSE, tidy=TRUE,  dependson=c("UKDD","UKDD2")}
...
To do so, we can  use `seasonaldummy()` in the `forecast()` package. For monthly data, this creates a matrix with a dummy variable for each month (0/1), with December as the reference. If our data were quarterly, it would create a dummy variable for each quarter.
```{r UKDD4, exercise=TRUE, exercise.eval=FALSE, tidy=TRUE,  dependson=c("UKDD")}
...
quiz(caption = "Quiz 2.2: Dummy variables and lag variables ", 
    ...
    answer("Controlling for time and seasonality will sometimes be enough to control for autocorrelation"),
    answer("If your data are yearly, seasonal dummy variables are not needed"),
    answer("A model can only incorporate either a step change, OR a change in slope, but not both", correct=TRUE),
    random_answer_order = FALSE, allow_retry = TRUE))
```
...
```{r UKDD5, exercise.eval=FALSE, exercise=TRUE, dependson=c("UKDD")}
...
```{r UKDD6, exercise=TRUE, exercise.eval=FALSE, fig.height=4, fig.width=5, dependson=c("UKDD")}
# Plot raw data
par(cex.lab=.9,cex.axis=.9,cex.main=.9)
...
```{r UKDD7, exercise=TRUE, exercise.eval=FALSE, fig.height=4, fig.width=5, dependson=c("UKDD"), }
...
## Practical 2: Setting up your data for ITS analysis
...
```{r exercise2_1,exercise=TRUE,exercise.lines=19}
...
```{r exercise2_2,exercise=TRUE,exercise.lines=9,fig.width=7,fig.height=8,exercise.lines=12}
...
# To plot decomposed series use:
plot(decompose(alp))
```
```{r exercise2_2-hint-2}
# To create seasonal dummy variables, use:
seasonaldummy(alp)
```
...
```{r exercise2_3,exercise=TRUE,fig.height=4, fig.width=5}
...
```{r exercise2_4,exercise=TRUE,exercise.lines=5}
...
# To plot a vertical line use "abline(v=2017.083, col= , lty= )"
```
...
par(cex.lab=.9,cex.axis=.9,cex.main=.9)
plot(UKDD, ylim=c(500,3000), ylab="Driver deaths", xlab="Month",main="Figure. Driver deaths in the UK (1975 to 1984)")
```
...
```{r UKDD8, exercise=TRUE, exercise.eval=FALSE, fig.width=5, fig.height=4, dependson="UKDD"}
# Check distribution
hist(UKDD)
```

Even though the number of deaths is a count, large counts (i.e. that are not bounded by zero) can be treated as continuous. Thus, we will use a Gaussian (Normal) distribution (rather than Poisson).
...
```{r UKDD9, exercise=TRUE, exercise.eval=FALSE, tidy=TRUE, dependson="UKDD"}
...
# Since we suspect there is monthly seasonality, we will specify lag=12
Box.test(UKDD, type='Ljung-Box', lag=12)
```
...
We will use `lm()` to estimate our model. Afterwards, we will check the fit using the residual plots and test for residual autocorrelation using the Ljung-Box test. 
```{r UKDD10, exercise=TRUE, exercise.eval=FALSE, fit.width=5, fit.height=5,  dependson=c("UKDD","UKDD2")}
# Estimate model
UKDD.model1 <- lm(UKDD ~ UKDD.time + belt + belt.after + UKDD.mon)
...
text_tbl <- data.frame(
  ...
  Estimate = c("2077 (1995, 2159)","-0.1 (-0.9, 0.7)","-363 (-469, -257)","5.8 (-1.3, 12.8)","1485.3"))
...
```{r UKDD12, exercise=TRUE, exercise.eval=FALSE, dependson=c("UKDD","UKDD2"),  tidy=TRUE}
...
**Table.** Summary of Models 1 and 2

```{r, eval=TRUE, echo=FALSE}
text_tbl <- data.frame(
  Param = c("Intercept","Baseline trend","Level shift","Change in trend","AIC"),
  Model1 =c("2077 (1995, 2159)","-0.1 (-0.9, 0.7)","-363 (-469, -257)","5.8 (-1.3, 12.8)","1485.3"),
  Model2 = c("2089 (2004, 2175)","-0.3 (-1.2, 0.5)","-338 (-451, -224)","5.6 (-2.3, 13.5)","1496.9"))
...
```{r UKDD13, exercise=TRUE, exercise.eval=FALSE, fig.width=6, fig.height=5, dependson=c("UKDD2","UKDD")}
...
The red line in this figure represents the number of deaths we would have expected to see if the seatbelt law had not been implemented.
...
```{r sp1, eval=TRUE, fig.width=8, fig.height=5, }
# Transform to time series object
sp <- ts(smallpox[,2], start=1750, frequency=1)

# Plot raw data
par(cex.lab=.9,cex.axis=.9,cex.main=.9)
plot(sp, col='navy', ylim=c(0,20), ylab='Deaths due to smallpox (%)', xlab='Year', main='Figure. Percentage of all deaths in London due to smallpox (1750-1831)')
abline(v=1796, col="red", lwd=2, lty="dashed")
```
...
```{r sp2, eval=TRUE, fig.width=8, fig.height=5, dependson="sp1"}
hist(sp, main="Histogram of smallpox deaths (%)")
```

Let's stabilise the variance by log transforming our series.
```{r sp3, eval=TRUE, exercise.eval=FALSE, fig.width=8, fig.height=5, dependson="sp1"}
# Plot log transformed time series
plot(log(sp), col='navy', ylim=c(0,4), ylab='Log of deaths (%) due to smallpox', xlab='Year', main='Figure. Percentage of all deaths in London due to smallpox (1750-1831)')
abline(v=1796, col="red", lwd=2, lty="dashed")

# Plot histogram of log transformed time series
hist(log(sp), main="Histogram of smallpox deaths,\nlog transformed")

```

That looks better. 

Now, let's create a vector representing the time since the start of the study to estimate the baseline slope, a step change, and a change in slope.
```{r sp4, exercise=TRUE, exercise.eval=FALSE,  dependson="sp1"}
...
First, let's try a linear regression, and make sure to check the residuals for the presence of autocorrelation. We've already seen the Ljung-Box test. 

We will use the log transformed outcome variable.
```{r sp5, exercise=TRUE, exercise.eval=FALSE, fig.width=6, fig.height=5,  dependson=c("sp1","sp4")}
...
Box.test(sp.model1$residuals, type="Ljung-Box", lag=1)

```

Hmm, the p-value for the Ljung-Box test is very small, indicating significant residual first-order autocorrelation in the model. Given that there is no trend prior to the intervention, and that there is no seasonality, we'll try an AR() model (instead of a full ARIMA model).
...
```{r sp6, exercise=TRUE, exercise.eval=FALSE, dependson=c("sp1","sp4")}
# Apply AR(1) model to data
auto.arima(log(sp), d=0, max.q=0, xreg=cbind(sp.time, sp.vax, sp.after))
```

This suggest an AR(1) model. Let's estimate the model using `sarima()` in the `astsa` package.
```{r s7, exercise=TRUE, exercise.eval=FALSE, dependson=c("sp1","sp4"), }
# Apply AR(1) model to data
sp.model2 <- sarima(log(sp), p=1, d=0, q=0, xreg=cbind(sp.time, sp.vax, sp.after))
```

`sarima()` automatically produces the residuals plots, with the Ljung-Box test at the bottom. As the dots are all above the blue line for the Ljung-Box statistic, this indicates no more autocorrelation. Our final model is an AR(1) model - that is, an autoregressive model with 1 order of autocorrelation. Let's get our final estimates:

```{r sp8, exercise=TRUE, exercise.eval=FALSE, dependson="sp7"}
sp.model2$fit
confint(sp.model2$fit)
```

Given that our outcome was log transformed, we will need to back transform the estimates using the
$(e^{estimate}-1)*100$, and the resulting estimates can be interpreted as a percentage change. Here are our estimates:
...
In January 2014, the PBS decided to eliminate repeat prescriptions for the lowest tablet strength ([Brett et al.](https://onlinelibrary.wiley.com/doi/full/10.1002/pds.4408)). 
...
```{r quet1, exercise=TRUE, exercise.eval=FALSE,fig.width=6, fig.height=4}
# Plot data
par(cex.lab=.9,cex.axis=.9,cex.main=.9)
...
abline(v=2014.00, col="gray", lwd=2, lty="dashed")

```
```{r quet1b, exercise=TRUE, exercise.eval=FALSE}
...
Now, let's create two vectors representing the intervention effects. Based on our visualisation, we hypothesise that the intervention results in a level shift (step change) and a change in slope (ramp).
```{r quet2, exercise=TRUE, exercise.eval=FALSE, dependson="quet1",  tidy=TRUE, fig.height=4,fig.width=6}

# Create vector for step change after change in subsidy
quet.step <- ts(append(rep(0,36), rep(1,12)), start=c(2011,1), frequency=12)
print(quet.step)

# Create vector for change in slope (ramp)
# Note that we have 36 months prior to the intervention, and 12 months post-intervention
quet.ramp <- ts(append(rep(0,36), seq(1,12)), start=c(2011,1), frequency=12)
print(quet.ramp)
...
```{r quet3, exercise=TRUE, exercise.eval=FALSE, fig.width=5, fig.height=4, dependson="quet1"}
# Plot the ACF/PACF plots - remember to use the differenced series
acf2(diff(diff(quet), lag=12))
```

Hmm, the ACF and PACF aren't particularly useful in this case. Let's check with `auto.arima()` - make sure to prespecify *d* and *D* based on what we found above. 
...
```{r quet4, exercise=TRUE, exercise.eval=FALSE, dependson="quet1"}
# Check for most appropriate AR/MA orders
auto.arima(quet, d=1, D=1, xreg=cbind(quet.step, quet.ramp), stepwise=FALSE)
```

`auto.arima()` suggests the best model based on the lowest information criteria (AIC, BIC). It suggests that our model is $(2,1,0)\times(0,1,1)_{12}$.

***

#### Step 4. Model Estimation 

Next, fit the proposed model and check the model fit using `sarima()`. Make sure to include the step and ramp functions when specifying the model - like before, you do this by including the argument `xreg=`.
```{r quet5, exercise=TRUE, exercise.eval=FALSE, fig.width=6, fig.height=7, dependson=c("quet1","quet2"),  tidy=TRUE}
# Model estimation
quet.model1 <- sarima(quet, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp))
```

The model fits looks okay, and the Ljung-Box test is nonsignificant at all lags. The QQ-plot is borderline, but the deviation is mostly at the extremes which is to be expected.

***

Now, let's get our final estimates:

```{r quet6, exercise=TRUE, exercise.eval=FALSE, dependson="quet5"}
# Get estimates and confidence intervals
quet.model1$fit
confint(quet.model1$fit)
```
...
```{r quet7, exercise=TRUE, exercise.eval=FALSE, dependson=c("quet1","quet5"), tidy=TRUE, fig.height=4, fig.width=6}
# Model data excluding post-intervention period, using the ARIMA
# orders identified previously.
# IMPORTANT: you need to use the "Arima()" function in the forecast package (*not* sarima or arima).
quet.model2 <- Arima(window(quet,end=c(2013,12)),order=c(2,1,0),seasonal=list(order=c(0,1,1), period=12))
...
quet.fc2 <- as.numeric(quet.fc$mean) %>% ts(start=c(2014,1), frequency=12)
...
# Plot
par(cex.lab=.9,cex.axis=.9,cex.main=.9)
plot(quet.cf, plot.type="s", ylab="No. dispensings", xlab="Month", ylim=c(0,40000), col=c("blue","maroon"),lty=c("solid","longdash"),main="Figure. Quetiapine dispensing in Australia (2011-2014)")
abline(v=2014.00, col="gray", lwd=2, lty="dashed")
legend("bottomright",legend=c("Observed","Predicted"),col=c("blue","goldenrod"),lty=c("solid","longdash"),cex=.8)
...
## Practical 3: Segmented regression and ARIMA 
...
```{r exercise3_1,exercise=TRUE, tidy=TRUE, exercise.eval=FALSE, exercise.lines=15, fig.width=5, fig.height=4}
...
# Check your model fit by plotting the residuals - what do these plots tell you?
...
# To model your time series:
model1 <- lm(alp ~ time + step + after + month)
```
```{r exercise3_1-hint-2}
# To output the model summary use:
summary(model1)
```
```{r exercise3_1-hint-3}
...
```{r exercise3_2,exercise=TRUE, tidy=TRUE, exercise.eval=FALSE, exercise.lines=12}
...
```{r exercise3_3,exercise=TRUE, exercise.eval=FALSE, exercise.lines=8}
# Rerun the model with the intervention variables delayed by 1 month (call it "model2")
model2 <-

# Check the summary statistics and AIC and compare it to the first model. How do the parameter estimates compare? Does this model have a better or worse fit?
...
Make sure to use the "best" model (model1 or model2) from above. 
```{r exercise3_4,exercise=TRUE, exercise.eval=FALSE, fig.height=4, fig.width=5, exercise.lines=25}
...
# Variables if intervention had not occurred:
noint <- data.frame(step=rep(0,length(alp)), after=rep(0,length(alp)))
...
```{r exercise3_5, exercise=TRUE, exercise.eval=FALSE, exercise.lines=10}
...
# To get estimates, use:
summary(model1)

```
```{r exercise3_5-hint-2}
...
#### Exercise 3.6 ARIMA
```{r exercise3_6,exercise=TRUE,exercise.eval=FALSE, exercise.lines=8}
...
# How do the parameter estimates compare to the segmented linear regression above?
