...
data(UKDriverDeaths)
UKDD <- window(UKDriverDeaths, start=c(1975,1)) %>% ts(start=c(1975,1), frequency=12)

belt <- append(rep(0,sum(as.yearmon(time(UKDD))<'Feb 1983')),rep(1,sum(as.yearmon(time(UKDD))>='Feb 1983'))) %>% ts(start=c(1975,1), frequency=12)
               
UKDD.time <- seq(1, length(UKDD)) %>% ts(start=c(1975,1), frequency=12)

belt.after <- append(rep(0,sum(as.yearmon(time(UKDD))<'Feb 1983')), seq(1,sum(as.yearmon(time(UKDD))>='Feb 1983')))%>% ts(start=c(1975,01), frequency=12)

belt.lag <- append(rep(0,sum(as.yearmon(time(UKDD))<'Mar 1983')),rep(1,sum(as.yearmon(time(UKDD))>='Mar 1983'))) %>% ts(start=c(1975,1), frequency=12)

belt.after.lag <- append(rep(0,sum(as.yearmon(time(UKDD))<'Mar 1983')),seq(1,sum(as.yearmon(time(UKDD))>='Mar 1983'))) %>% ts(start=c(1975,01), frequency=12)

UKDD.mon <- seasonaldummy(UKDD)

alprazolam <- read.csv(file="data/alprazolam.csv", header=TRUE)
alp <- window(ts(alprazolam, start=c(2014,7), end=c(2018,6), frequency=12 ), end=c(2018,1))
...
noslope <- data.frame(belt.after=rep(0, length(belt)))
UKDD.cf <- predict.lm(UKDD.model1, newdata=c(nobelt,noslope))
...
sp.vax <- append(rep(0,sum(time(sp)<'1796')), rep(1,sum(time(sp)>='1796')))
...
quet <- as.data.frame(quetiapine[,2]) %>% ts(start=c(2011,1), end=c(2014,12), frequency=12)
quet.step <- ts(append(rep(0,sum(as.yearmon(time(quet))<'Jan 2014')),rep(1,sum(as.yearmon(time(quet))>='Jan 2014'))), start=c(2011,1), frequency=12)
quet.ramp <- ts(append(rep(0,sum(as.yearmon(time(quet))<'Jan 2014')),seq(1,sum(as.yearmon(time(quet))>='Jan 2014'))), start=c(2011,1), frequency=12)

quet.model1 <- sarima(quet, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp), details=FALSE)
...
The reference text is Shadish WR, Cook TD, Campbell DT. Experimental and quasi-experimental designs for generalized causal inference. 2002. Boston: Houghton Mifflin. ISBN: 0395615569. [Chapter 6](https://github.com/CBDRH/HDAT9700/blob/master/inst/tutorials/its/Shadish_Chapter6.pdf)
...
![**Figure.** Examples of potential before and after scenarios. (Modified from Soumerai et al. 2015. *Prev Chronic Dis*)](images/before_after.png)
...
```{r UKdeaths, fig.height=4, fig.width=5, exercise=TRUE, exercise.eval=FALSE, include=TRUE, echo=TRUE}
...
```{r UKDD, eval=FALSE, fig.height=4, fig.width=5, exercise=TRUE, exercise.eval=FALSE, tidy=TRUE}
...
belt <- append(rep(0,sum(as.yearmon(time(UKDD))<='Jan 1983')), rep(1,sum(as.yearmon(time(UKDD))>'Jan 1983')))
...
It is important to only consider lagged or delayed effects when it makes sense from a practical or clinical perspective - given what you know about the intervention and outcome, is it possible that the impact could be delayed?

```{r shift, exercise=TRUE, exercise.eval=FALSE}
# Create vector for introduction of seatbelt law, lagged by one month
belt.lag <- append(rep(0,sum(as.yearmon(time(UKDD))<='Feb 1983')), rep(1,sum(as.yearmon(time(UKDD))>'Feb 1983'))) %>% ts(start=c(1975,01), frequency=12)
print(belt.lag)
...
time <- seq(1, length(alp))
```
```{r exercise2_1-hint-3}
# To create sequence variable for time since intervention use:
after <- append(rep(0,sum(as.yearmon(time(alp))<'Feb 2017')),seq(1,sum(as.yearmon(time(alp))>='Feb 2017'),1))
```
...
Here we see that the baseline trend was essentially zero. Immediately after the seatbelt law came into effect, there was a significant level shift (or step change) reduction in driver deaths. We can thus say that the intervention was associated with in an immediate reduction in deaths (-363, 95% CI -469 to -257), that was sustained for the duration of the study period. There was no change in trend. It is always a good idea to go back to your plot, and check that these numbers make sense.
...
```{r sp, eval=TRUE, fig.width=8, fig.height=5}
# Transform to time series object
sp <- ts(smallpox[,2], start=1750, frequency=1)

# Plot raw data
plot(sp, col='navy', ylim=c(0,20), ylab='Deaths due to smallpox (%)', xlab='Year', main='Percentage of all deaths in London due to smallpox (1750-1831)')
...
```{r sp.hist, eval=TRUE, fig.width=8, fig.height=5}
hist(sp, main="Histogram of smallpox deaths (%)")
```

Let's stabilise the variance by log tranforming our series.
```{r sp2, eval=TRUE, exercise.eval=FALSE, fig.width=8, fig.height=5}
# Plot log transformed time series
plot(log(sp), col='navy', ylim=c(0,4), ylab='Log of deaths (%) due to smallpox', xlab='Year', main='Percentage of all deaths in London due to smallpox (1750-1831)')
abline(v=1796, col="red", lwd=2, lty="dashed")
...
```{r sp3, exercise=TRUE, exercise.eval=FALSE}
...
```{r sp4, exercise=TRUE, exercise.eval=FALSE, fig.width=6, fig.height=5}
...
```{r sp5a, exercise=TRUE, exercise.eval=FALSE}
...
```{r sp5b, exercise=TRUE, exercise.eval=FALSE}
# Apply AR(1) model to data
sp.model2 <- sarima(log(sp), p=1, d=0, q=0, xreg=cbind(sp.time, sp.vax, sp.after))
```
...
```{r sp6, exercise=TRUE, exercise.eval=FALSE}
sp.model2$fit
...
* Baseline slope: -0.1% (-0.6% to 0.3%)
* Step change: 2.6% (-15.1% to 24.1%)
* Change in slope: -3.3% (-4.1% to -2.5%)

This means that prior to the vaccine, the proportion of deaths due to smallpox was stable (no change). After the vaccine was invented, there was no evidence of a step change in the number of deaths; however, the percentage of deaths due to smallpox decreased by 3.3% per year (95% CI -4.1% to -2.5%).
...
# Plot data
plot(quet, ylab="No. dispensings", xlab="Month", ylim=c(0,40000), main="Quetiapine dispensings in Australia", col="blue")
...
# Plot seasonal data
ggseasonplot(quet, color=rainbow(12), year.labels=TRUE)
...
Now, let's create two vectors representing the intervention effects. Based on our visualisation, we hypothesise that the intervention resultes in a level shift (step change) and a change in slope (ramp).
...
# Model your time series "alp" using a combination of a step change and slope change (call it "model1") - include month as a dummy variable 



# Model summary (using "summary()") - are both the step change and slope change significant?
...
# Conduct a Ljung-Box test of residuals for autocorrelation
...
# Rerun the model with this intervention variables delayed by 1 month (call it "model2")
...
Make sure to use the "best" model (model1 or model2) based on the AIC above. 
```{r exercise3_4, exercise.cap="Exercise 3.4", exercise=TRUE, exercise.eval=FALSE, fig.height=4, fig.width=5, exercise.lines=25 }
...
```{r exercise3_5, exercise=TRUE, exercise.cap="Exercise 3.5", exercise.eval=FALSE, exercise.lines=10}
...
#### Exercise 3.6 ARIMA
```{r exercise3_6, exercise=TRUE, exercise.cap="Exercise 3.6", exercise.eval=FALSE, exercise.lines=8}
# Run the same model using "auto.arima()" - remember you don't need the variable representing time or seasonality - and prespecify "d" and "D"




# How does this compare to the linear regression above?
