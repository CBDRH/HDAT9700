...
data(UKDriverDeaths)
UKDD <- window(UKDriverDeaths, start=c(1975,1)) %>% ts(start=c(1975,01), frequency=12)

belt <- as.numeric(as.yearmon(time(UKDD))>='Feb 1983') %>% ts(start=c(1975,1), frequency=12)

UKDD.time <- seq(from=1, to=length(UKDD)) %>% ts(start=c(1975,01), frequency=12)

belt.after <- ifelse(as.yearmon(time(UKDD))<'Feb 1983', 0, UKDD.time-sum(belt==0)) %>% ts(start=c(1975,01), frequency=12)

belt.lag <- as.numeric(as.yearmon(time(UKDD))>='Mar 1983') %>% ts(start=c(1975,01), frequency=12)

belt.after.lag <- ifelse(as.yearmon(time(UKDD))<'Mar 1983', 0, UKDD.time-sum(belt.lag==0)) %>% ts(start=c(1975,01), frequency=12)
...
UKDD.model1 <- glm(UKDD ~ UKDD.time + belt + belt.after+ UKDD.mon)
UKDD.model2 <- glm(UKDD ~ UKDD.time + belt + belt.after + UKDD.mon)

belt.after.lag <- ifelse(as.yearmon(time(UKDD))<'Mar 1983', 0, UKDD.time-sum(belt.lag==0)) %>% ts(start=c(1975,01), frequency=12)

UKDD.model3 <- glm(UKDD ~ UKDD.time + belt.lag + belt.after.lag + UKDD.mon )
...
quet.step <- ts(as.numeric(as.yearmon(time(quet))>='Jan 2014'), start=c(2011,1), frequency=12)
quet.ramp <- ts(as.numeric(append(rep(0,36), seq(1,12,1))), start=c(2011,1), frequency=12)
...
belt.lag <- append(rep(0,sum(as.Date(UKDD)<='1983-03-01')), rep(1,sum(as.Date(UKDD)>'1983-03-01'))) %>% ts(start=c(1975,01), frequency=12)
print(belt.lag)
...
Recall that one of the key assumptions of a linear regression is that the residuals are independent and not correlated. After fitting your segmented regression model, you need to check for autocorrelation of the residuals. The best way to do this is using the Durbin-Watson or Ljung-Box test, which we will cover later. 

If there is remaining autocorrelation after adjusting for trend and seasonality, you may have to incorporate autoregressive terms. Recall that in an autoregressive model, the outcome is predicted using past values of the outcome itself. If there is only first-order autocorrelation, then you can use the Prais-Winsten estimator, which we will see later. However, if there is higher order autocorrelation you may want to consider an ARIMA model which we will discuss in Lecture 3.
...
When performing an ITS analysis, you should indicate the date of the intervention together with the raw data. We'll do it below using `abline()`. 
...
step <- append(rep(0,sum(as.Date(UKDD)<'2017-02-01')),rep(1,sum(as.Date(alp)>='2017-02-01')))
...
* The p-value is very small, and thus we reject the null hypothesis of no autocorrelation. However, sometimes autocorrelation is simply the result of trend or seasonality, and once we adjust for these autocorrelation will no longer be a problem. Thus, once we fit our regression model we will retest our residuals for the presence of autocorrelation.
...
UKDD.model2 <- glm(UKDD ~ UKDD.time + belt.lag + belt.after.lag + UKDD.mon)
summary(UKDD.model2)
confint(UKDD.model2)
AIC(UKDD.model2)
```
...
text_tbl <- data.frame(
  ...
  Model2 = c("2099 (2012, 2186)","-0.5 (-1.3, 0.3)","-327 (-444, -210)","6.2 (-2.4, 14.8)","1530.5"))
...
```{r sp, eval=TRUE, fig.width=6, fig.height=5}
...
```{r sp.hist, eval=TRUE, fig.width=6, fig.height=5}
hist(sp)
```

Let's stabilise the variance by log tranforming our series.
```{r sp2, eval=TRUE, exercise.eval=FALSE, fig.width=6, fig.height=5}
...
acf(sp.model1$residuals)
```

Hmm, the p-value for the Ljung-Box test is very small, indicating significant residual first-order autocorrelation in the model. In the ACF and PACF plots, we see that there is significant negative autocorrelation at lag 1 (based on the PACF, the significant autocorrelation at lag 2 is entirely due to autocorrelation at lag 1). This suggest an AR(1) model. We should therefore try our linear regression adjusted for autocorrelation only. 

To do so, we will specify out AR model using `sarima()` in the `astsa` package. We will start with specifying an AR(1) model, with p=1. Since this is an AR model, we also specify that d=0 and q=0. We also specify the regression variables using `xreg=`. 

```{r sp5, exercise=TRUE, exercise.eval=FALSE}
# Apply Prais-Winsten estimator to original model 
sp.model2 <- sarima(log(sp), p=1, d=0, q=0, xreg=cbind(sp.time, sp.vax, sp.after))
```

`sarima()` automatically produces the residuals plots, with the Ljung-Box test at the bottom. As the dots are all above the blue line for the Ljung-Box statistic, this indicates no autocorrelation at any lag. Our final model is an AR(1) model - that is, an autoregressive model with 1 order of autocorrelation. Let's get our final estimates:
...
Given that our outcome was log transformed, we will need to back transform the estimates using the (exp(estimate)-1)*100, and the resulting estimates can be interpreted as a percentage change. Here are our estimates:

* Baseline slope: -0.1% (-0.7% to 0.3%)
* Step change: 2.6% (-16.4% to 21.6%)
* Change in slope: -3.4% (-4.2% to -2.5%)

This means that prior to the vaccine, the proportion of deaths due to smallpox was stable. After the vaccine was invented, there was no evidence of a step change in the number of deaths; however, the percentage of deaths due to smallpox decreased by 3.4% per year (95% CI -4.2% to -2.5%).
...
One option is the autoregressive model described above, which is a specific type of ARIMA model. Here we will discuss full ARIMA models, where the trend is eliminated via differencing, rather than including time as a covariate in the model, which is only appropriate with linear trends.
...
In January 2014, the PBS decided to eliminate repeat prescriptions for the lowest tablet strength (Brett et al. [The impact of permissive and restrictive pharmaceutical policies on quetiapine dispensing: Evaluating a policy pendulum using interrupted time series analysis.](https://onlinelibrary.wiley.com/doi/full/10.1002/pds.4408)). 
...
auto.arima(quet, d=1, D=1, xreg=cbind(quet.step, quet.ramp), ic="bic", stepwise=FALSE)
auto.arima(quet, d=1, D=1, xreg=cbind(quet.step, quet.ramp), ic="aic", stepwise=FALSE)
```

Both AIC and BIC suggest that the model is $(2,1,0)\times(0,1,1)_{12}$.

***

#### Step 4. Model Estimation 

Next, fit the proposed model and check the model fit. Make sure to include the step and ramp functions when specifying the model - like before, you do this by including the argument `xreg=`.

```{r sarima1, exercise=TRUE, exercise.eval=FALSE, fig.width=6, fig.height=7}
# Model estimation
sarima(quet, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp))
```

The model fits looks good.
...
# Conduct a Ljung-Box test for autocorrelation
