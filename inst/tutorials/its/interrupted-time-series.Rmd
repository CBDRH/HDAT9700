...
text_tbl <- data.frame(
  ...
  Model2 = c("2090 (2004, 2176)","-0.3 (-1.2, 0.5)","-338 (-458, -219)","5.2 (-2.7, 13.1)","1497.2"))
...
To do so, we will specify an AR model using `sarima()` in the `astsa` package. We will start with specifying an AR(1) model, with *p*=1. Since this is an AR model, we also specify that *d*=0 and *q*=0 (recall from last chapter that *p* is the AR order of the model, *d* is the order of differencing, and *q* is the MA order). We also include the regression variables using `xreg=`. 
