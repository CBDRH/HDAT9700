...
UKDD.model1 <- glm(UKDD ~ UKDD.time + belt +  UKDD.after+ UKDD.mon)
UKDD.lag <- data.table::shift(UKDD, type="lag")
UKDD.model2 <- glm(UKDD ~ UKDD.time + belt + UKDD.after + UKDD.mon + UKDD.lag)

UKDD.after.lag <- ifelse(as.yearmon(time(UKDD))<'Mar 1983', 0, UKDD.time-sum(belt.lag==0))
UKDD.after.lag <- ts(UKDD.after.lag, start=c(1975,01), frequency=12)

UKDD.model3 <- glm(UKDD ~ UKDD.time + belt.lag + UKDD.after.lag + UKDD.mon + UKDD.lag)
...
quet <- as.data.frame(quetiapine[,2])
quet.ts <- ts(quet, start=c(2011,1), end=c(2014,12), frequency=12)
quet.step <- ts(as.numeric(as.yearmon(time(quet.ts))>='Jan 2014'), start=c(2011,1), frequency=12)
quet.ramp <- ts(as.numeric(append(rep(0,36), seq(1,12,1))), start=c(2011,1), frequency=12)

quet.model1 <- sarima(quet.ts, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp), details=FALSE)
...
To do so, we can  use `seasonaldummy()` in the `forecast()` package. This creates a matrix with a dummy variable for each month (0/1), with December as the reference. Try both methods to compare.
...
# Rerun the model 
UKKDD.model3 <- glm(UKDD ~ UKDD.time + belt.lag + UKDD.after.lag + UKDD.mon + UKDD.lag)
summary(UKDD.model3)
confint(UKDD.model3)
```
...
Let's use the time series `quet.ts`. It includes the monthly dispensings for the antipsychotic quetiapine in Australia from January 2011 to May 2014. Quetiapine is subsidised by the PBS for treatment of schizophrenia and bipolar I disorder. The lowest strength tablet is meant to be used for dose titration, but is commonly prescribed off-label for conditions such as insomnia. Note that "off-label" means prescribed for an indication other than the one for which it is approved. In January 2014, the PBS decided to eliminate repeat prescriptions for the lowest tablet strength (Brett et al. [The impact of permissive and restrictive pharmaceutical policies on quetiapine dispensing: Evaluating a policy pendulum using interrupted time series analysis.](https://onlinelibrary.wiley.com/doi/full/10.1002/pds.4408)). 
...
# Plot data
plot(quet.ts, ylab="No. dispensings", xlab="Month", ylim=c(0,40000), main="Quetiapine dispensings in Australia", col="blue")
abline(v=2014.00, col="gray", lwd=2, lty="dashed")

# Plot decomposed data
plot(decompose(quet.ts))

# Plot seasonal data
ggseasonplot(quet.ts, color=rainbow(12), year.labels=TRUE)
...
* The ACF plot shows that there autocorrelation, however the seasonality is not evident here. However, given that we know PBS data are typically seasonal, and we should try including a seasonal difference (*D*).
...
quet.ramp <- ts(append(rep(0,36), seq(1,12,1)), start=c(2011,1), frequency=12)
print(quet.ramp)
...
# Model estimation
sarima(quet.ts, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp))
```
...
# Model estimation 
quet.model1 <- sarima(quet.ts, p=2, d=1, q=0, P=0, D=1, Q=1, S=12, xreg=cbind(quet.step, quet.ramp))
