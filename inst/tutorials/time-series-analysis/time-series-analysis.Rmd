...
resp_ts <- ts(resp, start=c(1974,1), frequency=12)
...
The reference text for this chapter is "Time series analysis and its applications: with R examples" (Chapters 1-3) by Robert H Shumway and David S Stoffer, which is available through the [UNSW library](https://ebookcentral-proquest-com.wwwproxy1.library.unsw.edu.au/lib/unsw/detail.action?docID=3066093).  David S Stoffer is also the author of the `astsa` R package (**A**pplied **S**tatistical **T**ime **S**eries **A**nalysis)

Some other useful resources:

* Rob J Hyndman and George Athanasopoulos (Monash University) [Forecasting: Principles and Practice](https://otexts.org/fpp2/)
...
In the context of health data, one of the advantages of time series data is that they often involve aggregate data (such as counts of hospital separations), and thus they don't necessarily require access to identifiable person-level data. Thus, these data are often easier to access, and there are currently a large amount of freely avaiable time series data available online. 

Time series data have several unique properties. The ordering is important, and the data cannot be rearranged. In contrast to other types of data, time series data are also well suited to visualisation. Most importantly, observations made at neighbouring time points are often correlated and thus not independent, meaning that they do not meet the assumptions necessary for standard regression analysis. Therefore, specialised methods for analysing time series data are needed.
...
quiz(caption = "Quiz 1.1: Definition of time series data", 
    ...
    answer("Number of illict drug related ambulance calls per week over two years"),
...
# Create a vector with 48 observations taking values between 40 and 80
set.seed(123)
...
plot(mort, plot.type="s", ylab="Deaths", xlab="Week", col=c("blue","red"), lty=c("solid","dashed"), ylim=c(0, 150), main="LA Respiratory and Cardiovascular Mortality, 1970-1979")
# Add a legend
legend("topright", legend=c("Respiratory","Cardiovascular"), col=c("blue","red"), lty=c("solid","dashed"), cex=.8)
```
...
Now we know that the series starts in January 1968 and ends in December 1978, and that the data are monthly (frequency = 12). The maximum value is 0.82 and the minimum is 0.18. 
...
# Plot flu data
plot(flu, ylab="Deaths per 10,000 people", xlab="Month", main="Pneumonia and influenza deaths in the US (1968-1978)", ylim=c(0,1))
```

What are the key features of this time series?

* **Trend:** It's hard to tell if there is a trend, but overall rates appear to be decreasing over time. 
* **Seasonality:** These data are highly seasonal, with pneumonia and influenza deaths more common in the winter than in the summer which is what we'd expect. 
...
* **Seasonality**: There is no seasonality. However, this is because the data are yearly. If we were to look at monthly or quarterly data, we'd see seasonal effects with higher rates in summer and lower in winter. 
...
quiz(caption = "Quiz 1.4: Seasonality", 
    ...
    answer("Series 1, 2 and 4", correct=TRUE, message="Series 1 is expenditure on eating out in Australia, Series 2 is births in the US, and Series 4 is daily electricity demand in Victoria"),
...
Now let's check the Hawaiian occupancy rates data. A second option to compare rates across seasons is to use the function `tapply()`. We can use `INDEX=summary` to request the summary statistics (min, median, mean, IQR, max) for the `hor` data. We also specify `INDEX=cycle(hor)`, which tells R that we want the summary by season, in this case quarters.
...
* **Additive decomposition:** Use an additive model to decompose the time series if the variance is constant. An additive model is expressed as:
...
# Plot them both on the same graph
plot(h02_t, plot.type="s", ylim=c(0,1.5), col=c("blue","red"), lty=c("solid","dashed"), ylab="Corticosteroid Rx (millions)", xlab="Month")
...
# Check if resp is a ts object
class( )

# If not, convert to a ts object and call the new ts object "resp_ts"
...
# To plot on the graph, use the argument "plot.type="s"""
```
...
# To combine the raw data with the trend data, use "ts.union(resp[,1], males$trend)"
```
...
# To calculate the ratio, use "ratio <- resp[,1]/resp[,2]"
```
***
...
Let $Y_t$ represent the values of your time series ($Y$) at times $t$, from $t_1$ through $t_n$, where $n$ is the number of observations in your series.
...
Why is stationarity important? A stationary series is easier to work with because it is parsimonious and stable over time; that is, it can be modelled with fewer parameters. While a stationarity series may fluctuate, it will always revert to a constant mean, meaning that it is easier to predict. However, a strictly stationary series is rare, and it is often enough to assume *weak stationarity*. 
...
There is virtually no autocorrelation present in the Hawaiian occupancy data (r = 0.13), whereas there is near perfect correlation in the international visitor nights data (r=0.92)
...
Here there is no significant partial autocorrelation. Therefore, the higher order autocorrelation observed in the ACF plot above was entirely due to lower order autocorrelation. 
...
plot(h02_yr, ylim=c(0,15), ylab="Corticosteroid Rx (millions)", xlab="Year")
```
...
# Use aggregate( , nfrequency=1) to aggregate the data
...
Here, $c$ is a constant, $\phi$ is the magnitude of the autocorrelation (between -1 and 1), $p$ is the number of lags, and $\epsilon_t$ is the white noise.
...
Here $c$ is a constant, $\theta$ is the value of the autocorrelation (between -1 and 1), and $q$ is the number of lags. 
...
Here $c$ is a constant, $\Phi$ is the value of the autocorrelation (between -1 and 1), and $s$ is the seasonality (e.g. 12 for monthly, 4 for quarterly). 
...
See the following [website](https://people.duke.edu/~rnau/411arim3.htm) for some useful information for identifying the order of ARIMA models.
...
*z* is a simulated time series generated from the following AR(2) model:
...
We can see that the PACF plot cuts off sharply at lag 2, while the ACF plot tails off. This indicates that this is an AR model, and that *p* = 2.
...
In this case, it's not entirely clear! It looks like the PACF plot might be tailing off, which suggests an MA model. But the seasonality is clear.
...
Given that different information criterion can give different results, we'll check all of them separately and choose the simplest of the three models.
...
auto.arima(log(AirPassengers), d=1, D=1, ic="aicc")
```
...
# Check for stationarity by plotting the data, and using adf.test() and kpss.test()
```
...
# Consider which parameters to pre-specify
auto.arima(p,    )

```
```{r exercise3_2-hint-1}
# Prespecify "d" and "D" based on what you observed in your stationary test, and in the ACF/PACF plots
```
