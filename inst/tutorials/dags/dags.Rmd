...
In this chapter you will learn how to represent assumptions about casual mechanisms using a special type of graph: a Directed Acyclic Graph, or DAG. DAGs are useful because they allow us to formalise our assumptions and can help inform variable selection for causal models. 
...
This definition introduces two new concepts that we also have to define: _backdoor_ paths, and what makes an _open_ or _closed_ path.

#### 1. Backdoor paths
**Definition:** A backdoor path between X and Y is a path between X and Y that contains an arrow pointing towards Y. 

For example, the path ice-cream sales &larr; sunshine &rarr; sunburn is a backdoor path between ice-cream sales and sunburn.
...
![](https://youtu.be/gIeQymUfyBM)

## Worked example

In this section we will use the principle of d-separation and backdoor paths to identify a minimal set of adjustment variables that will allow us to estimate the effect of Exercise on Mood based on the following DAG. 
...
In order to estimate the **total** effect of exercise on mood we need to close all the available backdoor paths. If we are interested in the **direct** effect of mood on exercise we additionally need to close any frontdoor paths, other than the main causal pathway Exercise &rarr; Mood. So let's start by identifying all of the backdoor and frontdoor paths between Exercise and Mood.

#### Backdoor paths 
To identify backdoor paths, look for paths between the exposure and outcome that have one or more arrows pointing toward the exposure. In total there are three backdoor paths
...
#### Frontdoor paths 
To identify frontdoor paths, look for paths between the exposure and outcome where all arrows point towards the outcome. There are two frontdoor paths are: 
...
* To close the path Exercise &larr; Club member &rarr; Social interactions &rarr; Mood we could control for Club membership OR we could control for number of social interactions. Note that controlling for either variable is sufficient to close the path. We could also control for both variables; this isn't necessary to close the path, but it won't do any harm. 
...
We identified two front door paths. The first was the path Exercise &rarr; Mood. This is the causal path of interest so we don't have to do anything here (obviously we will include our exposure and outcome in the model).

The second frontdoor path was Exercise &rarr; Sleep quality &rarr; Mood. This path implies that part of the effect of exercise is mediated through sleep quality. In order to estimate the direct effect of exercise we must close this pathway, i.e. control for sleep quality in our analysis.
...
### Activity 1. Test your understanding of d-separation and closing backdoor paths
Explore the concept of d-separation and backdoor paths adjustment further using this [interactive Shiny app](https://cbdrh.shinyapps.io/daggle/). The app can also be launched by entering `hdat9700tutorials::run('backdoor-paths')` at the console. 

This app will generate a random DAG and you must identify which variables to control for in order to estimate causal effect of the exposure on the outcome. You can vary the DAG complexity, and choose to focus on the total effect or the direct effect (you should have a go using both modes). 
...
g %>%
  dagitty::dagitty() %>%
  dag_label(labels = c(X = "Mask", Y = "COVID-19", Z1 = "Compliant", Z2 = "Participation")) %>%
  adjust_for(NULL) %>% 
  node_status() %>% 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend, fill = status, shape = adjusted)) +
  geom_dag_point(aes(color = adjusted)) +
...
When we estimated the effect of mask wearing on COVID-19 using data from participants only, we are effectively controlling for participation status. To see this, consider the  hypothetical scenario where data is available for participants and non-participants alike. In this case, stratifying on participation status and performing the analysis among survey respondents only is the same as only having data for survey respondents in the first place. 

Looking at the DAG above, it should be clear that survey participation is a collider on the path Mask &rarr; Compliant &rarr; Participation &larr; COVID-19. And what happens when we condition on a collider? As you have learned in the section on Collider Variables, controlling for response status will **open** the triplet Compliant &rarr; Participation &larr; COVID-19. In addition, assuming compliant personality type is a latent, unobserved variable, we will be unable to control for this variable.  Thus there will be an open backdoor pathway between the exposure and outcome.
