...
description: "Learn how to use Directed Acyclic Graphs (DAGs) as a way to formalise expert knowledge, represent assumptions about causal relationships, and inform variable selection when addressing causal research questions." 
---
...
# Define colors for DAGs
exposureCol <- "#CDDC55"
outcomeCol <- "#4FBAE4"
# unadjustedCol <- "seagreen"
adjustedCol <- "pink"
naCol <- 'grey80'
...
<!-- ![](https://youtu.be/lhkmc-dXC7M) -->

In this chapter you will learn how to represent assumptions about casual mechanisms using a special type of graph: a Directed Acyclic Graph, or DAG. DAGs are useful because they allow us to formalise and communicate causal assumptions underlying a particular research question and can help inform variable selection for causal models. 
...
* [Austin, Anna E., Tania A. Desrosiers, and Meghan E. Shanahan. Directed Acyclic Graphs: An under-utilized tool for child maltreatment research. Child Abuse & Neglect 91 (2019): 78-87](https://doi.org/10.1016/j.chiabu.2019.02.011)

* [Hanly, M., Brew, B. K., Austin, A., & Jorm, L. (2023). Software Application Profile: The daggle app—a tool to support learning and teaching the graphical rules of selecting adjustment variables using directed acyclic graphs. International Journal of Epidemiology, 52(5), 1659-1664](https://doi.org/10.1093/ije/dyad038)

### Additional resources: 

* [The daggle app](https://cbdrh.shinyapps.io/daggle/): A shiny app to learn and practice the rules of covariate selection using a DAG

* [dagitty.net](http://www.dagitty.net/learn/): A great website explaining DAGs and related topics, with built-in applets to test your knowledge
...
The statistical analyses you will perform as Health Data Scientists can be categorised according to three broad tasks of data science. 
...
Causal inference tasks are often referred to as "Explanation" (see the video [To Explain or to Predict?](https://www.youtube.com/watch?v=vWH_HNfQVRI&t=1s) for example). However, Hernán et al (2019) emphasise that **quantifying causal effects doesn't necessarily equate to explaining causal mechanisms**. This is a fair point: knowing that smoking causes cancer is different to knowing which chemicals are carcinogenic, for example. 
...
The absence of an arrow between two variables represents the assumption that neither neither variable is causing or caused by the other. In the next DAG, **Z** causes **X** and **Z** causes **Y**, but **X** and **Y** are _causally_ unrelated (there is no arrow pointing from **X** to **Y** or from **Y** to X). 
...
As we will discuss at length, the absence of an arrow between two nodes on a DAG, doesn't mean that the variables are uncorrelated in an observational dataset. What if our DAG above represented **sunshine**, **icecream sales** and **sunburn**?
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c("X" = "Ice-cream sales", "Y" = "Sunburn", "Z" = "Sunshine")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
We know that icecream sales definitely don't cause sunburn, but you wouldn't be surprised to find an association between ice-cream sales and incidence of sunburn (a case of correlation not implying causation!). This is a classic example of **confounding bias**. The relationship between ice-cream sales and sunburn is **confounded** by sunshine, inducing an association between the two variables, even though there is no causal relationship.

By using expert knowledge to express our qualitative understanding of causal relationships of interest, DAGs help us to distinguish between correlation and causation.
...
As an example, let's say we are interested in the causal relationship between between aspirin and pulmonary embolism, in a setting where aspirin is more often prescribed to patients with high blood pressure. In this case, high blood pressure is a **common cause** of both aspirin usage and pulmonary embolism: patients with high blood pressure are more likely to be taking aspirin and high blood pressure increases the risk of a pulmonary embolism. To satisfy the causal Markov assumption, blood pressure should be included in the DAG (and controlled for in any corresponding analysis).
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X = "Aspirin", Y = "Pulmonary embolism", Z = "Blood pressure")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c("X" = "Aspirin", "Y" = "Pulmonary embolism")) |>
  adjust_for(NULL) |>
  node_status() |>
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X="exercise", Y = "melanoma", Z1="health consciousness", Z2="sun exposure", Z3="screening")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
    status = factor(
      ifelse(is.na(status), 3, status),
      levels = 1:3,
...
* exercise $\rightarrow$ melanoma (The direct path - this doesn't seem like a very plausible relationship but it could be the hypothesis the investigator wants to test)

#### Parent
The parents for a given node in a DAG are the set of variables with an arrow pointing directly into that node. For example, a parent of **melanoma** is **sun exposure**.
...
quiz(caption = "Quiz: DAGS",
  question("Which of the diagrams above is not a valid DAG?",
    answer("A"),
    answer("B", correct = TRUE, message = "Correct, (B) is not a valid DAG because it is not _acyclic_. If you start at **X** you can follow the direction of the arrows to get back to **X** again, making this a cyclic graph."),
    answer("C"),
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X = "Maternal age", Y = "Low birthweight", Z = "Social deprivation")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
<!-- ## Video: Common causal structures -->

<!-- ![](https://youtu.be/DhHBMvpYeec) -->

<!-- Learn how to represent confounding variables, mediating variables and collider variables on a DAG. -->
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X = "Maternal age", Y = "Birth weight", Z = "Socioeconomic status")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
### Definition
A mediating variable is a variable that lies on the causal pathway between an exposure **X** and outcome **Y**.
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X = "Exercise", Y = "Mood", Z = "Sleep quality")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
Here there is no connecting arrow going from exercise to mood. Our DAG reflects the assumption that _all_ of the effect of exercise is mediated through quality of sleep. This would be an indirect effect of exercise on mood.

However, we might also hypothesis a direct effect, perhaps caused by the release of endorphins. In that case we would have both an indirect path Exercise &rarr; Sleep quality &rarr; Mood and a direct path Exercise &rarr; Mood.
...
### Example
Suppose a researcher is interested in whether respiratory syncytial virus (RSV) infection during infancy (exposure) causes asthma in early childhood (outcome). In this case, admissions to the Emergency Department (ED) during early childhood would be a potential collider, because both RSV and asthma will increase the probability of ED admission during early childhood.
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X = "RSV infection", Y = "Asthma", Z = "ED admission")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
As we will see, identifying colliders is important because controlling for a collider can inadvertently introduce bias. To continue the above example, even though the number of ED admissions during early childhood may be correlated with both the exposure and outcome, controlling for this variable would actually _**bias**_ estimates of the relationship between RSV infection and asthma.
...
**Definition:** Two variables in a DAG are said to be d-separated if there are no open non-causal paths between them.

This definition introduces two new concepts that we also have to define: _causal and non-causal_ paths, and what makes an _open_ or _closed_ path.

#### 1a. Causal paths

A causal path is any path from the exposure to the outcome where all arrows point from the exposure towards the outcome. The causal effect of interest $\text{X} \rightarrow \text{Y}$  is one causal path, but if mediators are present there will be more than one causal path. In the mediation example below, both pathways are causal paths because they start with an arrow pointing from the exposure (Exercise) and end up at the outcome (Mood).
...
Both paths are causal paths:

* Exercise &rarr; Sleep quality &rarr; Mood
...
A non-causal path between an exposure **X** and outcome **Y** is a path between X and Y that contains one or more arrows pointing back towards **X**. A backdoor path is a type of non-causal path where an arrow points back into the exposure **X**.

For example, the path ice-cream sales &larr; sunshine &rarr; sunburn is a non-causal path between ice-cream sales and sunburn. This is also an example of a backdoor path .
...
You will recognise this as an example of confounding. In this case, the presence of sunshine induces a correlation or _dependence_ between ice-cream sales and sunburn. The "d" in d-connected stands for **dependence**!


#### 2. Open and closed paths

A path between two nodes X and Y is **open** if the path carries information or dependence between X and Y. We can open or close a path between two nodes by controlling for intermediate variables. In most practical contexts, controlling for a variable will mean stratifying by that variable in your analysis or adding the variable to a regression model.
...
1. Causal and non-causal paths
2. Open and closed paths

To estimate the TOTAL effect of X on Y you need to control for a set of variables that closes all non-causal paths, while leaving all causal paths open.

To estimate the DIRECT effect of X on Y you need to control for a set of variables that closes all non-causal paths and closes any causal paths except the path $\text{X} \rightarrow \text{Y}$.


## Worked example

In this section we will use the principle of d-separation and causal paths to identify a minimal set of adjustment variables that will allow us to estimate the effect of Exercise on Mood based on the following DAG.
...
g |>
  ...
  geom_dag_point(aes(color = status)) +
  geom_dag_edges(arrow_directed = grid::arrow(length = grid::unit(10, "pt"), type = "closed")) +
  geom_dag_label_repel(aes(label = label, fill = status), show.legend = FALSE, box.padding = 4, segment.color = 'grey80', size=9/.pt) +
  theme_dag(legend.position = 'bottom') +
...
This DAG represents several plausible assumptions:
...
### Identifying non-causal paths
In order to estimate the **total** effect of exercise on mood we need to close all open non-causal paths. If we are interested in the **direct** effect of mood on exercise we additionally need to close any open causal paths, other than the main causal pathway Exercise &rarr; Mood. So let's start by identifying all of the causal and non-causal paths between Exercise and Mood.

#### Non-causal paths
To identify non-causal paths, look for paths between the exposure and outcome that have one or more arrows pointing back toward the exposure. In total there are non-causal paths

* Exercise &larr; Club member &rarr; Social interactions &rarr; Mood
* Exercise &larr; Season &rarr; Mood
* Exercise &rarr; Blood pressure &larr; Mood


#### Causal paths
To identify causal paths, look for paths between the exposure and outcome where all arrows point towards the outcome. There are two casual paths:
...
#### Total effect
To estimate the total effect of Exercise on Mood we must close all of the non-causal paths. **Note that to close a path it is sufficient to control for any single variable along that path.** This means that for long paths with more than three nodes we can break the path up into triplets; if any triplet is closed then the whole path is closed.

* To close the path Exercise &larr; Club member &rarr; Social interactions &rarr; Mood we could control for Club membership OR we could control for number of social interactions. Here is an example where controlling for either variable is sufficient to close the path. We could also control for both variables; this isn't necessary to close the path, but it won't do any harm.

* To close the path Exercise &larr; Season &rarr; Mood we would need to control for Season.

* The path Exercise &rarr; Sleep quality &larr; Mood is already closed (because Sleep quality is a collider on this path) so we don't have to worry about this.
...
g |>
  ...
  mutate(
    status = factor(
      ifelse(adjusted=='adjusted', 3, 
             ifelse(adjusted=='unadjusted' & is.na(status), 4, status)),
      levels = 1:4,
      labels = c('exposure', 'outcome', 'adjusted', 'unadjusted')
    )) |> 
  ggplot(aes(x = x, y = y, xend = xend, yend = yend)) +
  geom_dag_point(aes(color = status, shape = status)) +
  geom_dag_edges(arrow_directed = grid::arrow(length = grid::unit(10, "pt"), type = "closed")) +
  geom_dag_label_repel(aes(label = label, fill = status), show.legend = FALSE, box.padding = 4, segment.color = 'grey80', size=9/.pt) +
  theme_dag(legend.position = 'bottom') +
  guides(color = guide_legend(override.aes = list(size = 9))) +
  scale_y_continuous(limits = c(0.8, 5.2)) +
  scale_fill_manual(NULL,
                    values = c('exposure' = exposureCol, 'outcome' = outcomeCol, adjusted = adjustedCol, unadjusted = naCol),
                    labels = c('exposure' = 'Exposure', 'outcome' = 'Outcome', 'adjusted' = 'adjusted', 'unadjusted' = 'unadjusted')) +
  scale_color_manual(NULL,
                     values = c('exposure' = exposureCol, 'outcome' = outcomeCol, adjusted = adjustedCol, unadjusted = naCol),
                     labels = c('exposure' = 'Exposure', 'outcome' = 'Outcome', 'adjusted' = 'adjusted', 'unadjusted' = 'unadjusted')) +
  scale_shape_manual(NULL,
                     values = c('circle', 'circle', 'square', 'circle'),
                     labels = c('exposure' = 'Exposure', 'outcome' = 'Outcome', 'adjusted' = 'adjusted', 'unadjusted' = 'unadjusted')
                     ) 
...
#### Direct effect
To estimate the direct effect of Exercise on Mood we must also consider alternative causal paths other than the main path of interest.

We identified two causal paths. The first was the path Exercise &rarr; Mood. This is the causal path of interest so we don't have to do anything here (obviously we will include our exposure and outcome in the model!).

The second causal path was Exercise &rarr; Sleep quality &rarr; Mood. This path implies that part of the effect of exercise is mediated through sleep quality. In order to estimate the direct effect of exercise we must close this pathway, i.e. control for sleep quality in our analysis.
...
### Activity 1. Test your understanding of using a DAG to select adjustment covariates

Practice using a DAG to select adjustment covariates [interactive Shiny app](https://cbdrh.shinyapps.io/daggle/). 

The daggle app will generate a random DAG and you must identify which variables to control for in order to estimate the causal effect of interest (total or direct effect). You can vary the DAG complexity, and choose to focus on the total effect or the direct effect (you should have a go using both modes).
...
<!-- ### Activity 2. Explore how understanding causal structures is key to model specification -->
<!-- Explore how the assumptions about causal structures can guide our model specification using this [interactive Shiny app](https://cbdrh.shinyapps.io/dags-1/). The app can also be launched by entering `hdat9700tutorials::run('dags-1')` at the console.  -->

<!-- This app generates data for three variables: aspirin use (the exposure), pulmonary embolism (the outcome) and high-blood pressure ( a covariate) under two data generation mechanisms. Scenario A emulates having collected these three variables from a randomised control trial; Scenario  emulates having collected these three variables from observational data. You will see that although the available variables are the same in these two scenarios, the correct DAGs and model specifications are different. -->
...
Many different types of biases arise from mechanisms that result in some individuals being recorded in our data and available for analysis, while data for other individuals are unavailable. These biases are referred to as Selection Biases.
...
Suppose a researcher was interested in estimating the effect of wearing a face mask (X) on the risk on contracting COVID-19 (Y) during the pandemic using data from a random-digit-dialing telephone survey. Survey respondents were asked whether they wore a face mask in public during the last month and if they had tested positive for the coronavirus. Of course, there are numerous factors that we would need to include in our DAG to address this research question, including the number and type of contacts, individual characteristics like age, sex and smoking status, and the quality and correct use of masks. For the this example, we will zoom in on one aspect: survey participation.

There is a large literature documenting the many ways that survey respondents (those who agreed to participate in a survey) differ from survey nonrespondents (those who decline to participate or can't even be contacted). Like any survey we can assume this telephone survey suffered from a degree of nonresponse - not every sampled phone number provided a response. In this context, we might hypothesise that survey respondents are more likely to wear masks, a correlation arising as a result of a latent variable: conscientious personality type. Conscientious people may be more likely to wear masks and more likely to agree to a survey request, resulting in an association (although not a causal relationship) between mask wearing and survey participation.
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c(X = "Mask", Y = "COVID-19", Z1 = "Conscientious", Z2 = "Participation")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
  ...
  geom_dag_label_repel(aes(label = label, fill = status), show.legend = FALSE, box.padding = 4, segment.color = 'grey80') +
  theme_dag(legend.position = 'bottom') +
  scale_y_continuous(limits = c(2.8, 5.2)) +
  guides(color = guide_legend(override.aes = list(size = 9))) +  
  scale_color_manual(NULL,
                    values = c('exposure' = exposureCol, 'outcome' = outcomeCol),
                    labels = c('exposure' = 'Exposure', 'outcome' = 'Outcome'),
...
When we estimated the effect of mask wearing on COVID-19 using data from participants only, we are effectively controlling for participation status. To see this, consider the  hypothetical scenario where data is available for participants and non-participants alike. In this case, stratifying on participation status and performing the analysis among survey respondents only (i.e. "controlling" for participation in the analysis) is equivalent to only having data for survey respondents in the first place.

Looking at the DAG above, it should be clear that survey participation is a collider on the path Mask &rarr; Conscientious &rarr; Participation &larr; COVID-19. And what happens when we condition on a collider? As you have learned in the section on Collider Variables, controlling for response status will **open** the triplet Conscientious &rarr; Participation &larr; COVID-19. In addition, assuming compliant personality type is a latent, unobserved variable, we will be unable to control for this variable.  Thus there will be an open non-causal path between the exposure and outcome.
...
It should be clear by now that adjusting for m would open a non-causal path between a and y in this DAG, as below:
...
g |>
  dagitty::dagitty() |>
  dag_label(labels = c("X" = "SSRI use", "Y" = "Lung cancer", "Z1" = "Depression", "Z2" = "Ever smoker", "Z3" = "Coronary disease")) |>
  adjust_for(NULL) |>
  node_status() |>
  pull_dag_data() |> 
  mutate(
...
g |>
  ...
  geom_dag_edges(arrow_directed = grid::arrow(length = grid::unit(10, "pt"), type = "closed")) +
  geom_dag_label_repel(aes(label = label, fill = status), show.legend = FALSE, box.padding = 4, segment.color = 'grey80') +
  geom_dag_collider_edges(color = 'pink', curvature=-0.15) +
...
# Try passing the model object to ggdag_status() instead of ggdag(). What's different?
...
For more complex DAGs, or mode control over colour and placement, it might be necessary to use the bespoke `geom_dag_*` functions that come with `ggdag` to tailor the elements of your desired DAG. Click through the slides below to see an example of this in action.
...
* The function `dagify()` is used to define the relationships between nodes. E.g. `y ~ x` indicates that an arrow will point from **x** to **y**.
...
DAGs are a useful tool that allow us to formalise our assumptions about causal mechanisms under study. Doing so has many advantages:
...
In order to estimated the causal path between an exposure X and an outcome Y, the nodes X and Y should be **d-separated**. This means that there should be **no open non-causal paths** between X and Y.
