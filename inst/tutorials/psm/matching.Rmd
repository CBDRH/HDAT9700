---
title: "HDAT9700: Matching for Causal Inference"
tutorial:
  id: "au.edu.unsw.cbdrh.hdat9700.tutorials.matching"
output:
  learnr::tutorial:
    progressive: false
    allow_skip: true
...
description: "Review the conditions required to make causal inferencees from observational data and implement matching as a pre-processing approach to uncover randomised studies hidden within observational datasets." 
---
...
### Discussions points
For the children listed in the table, 4/10 have on-track development if nobody participates in the program. If everybody participates then 6/10 have on-track development.
...
## Randomisation 
Classical random experiments have three core characteristics:  
1. Large $N$  
...
g %>%
  dagitty::dagitty() %>%
  dag_label(labels = c(A="Helping hands participation", Y="Development at age five", L="Pre-existing family issues")) %>%
  adjust_for(NULL) %>% 
  node_status() %>% 
...
There are some striking differences between the demographic characteristics of the treated and untreated groups. What are the implications when trying to evaluate the effectiveness of the program? **Could you draw a DAG to represent your assumptions about the underlying causal structure** relevant to the causal question of treatment participation on earnings?

(Keep in mind the cultural context---USA in the mid 1970s)
...
(ii) It takes into account the **covariance** structure in the data when calculating distance. To see what this means, consider the points **a** and **b** on the scatterplot of education versus salary below.
...
The example below matches treated and untreated units based on the variables *race* (Black, Hispanic or White) and *re74* (income in 1974). The matching method is 1:1 nearest neghbour matching, based on the propensity score. 

``` {r ex4, exercise=FALSE, eval=TRUE, echo=TRUE}
match1 <- matchit(treat ~ race + re74, data=lalonde,
...
### Assessing balance with the Cobalt package
...
Although `matchit` does provide functionality to compare balance, there are much nice options available through the `cobalt` package. `cobalt` stands for **Co**variate **Bal**ance **T**ables (and Plots) and was developed specifically to help compare balance on matched data. You can read more about `cobalt` on it's GitHub page [here](https://ngreifer.github.io/cobalt/) and check out this article on [comparing balance](https://cran.r-project.org/web/packages/MatchIt/vignettes/assessing-balance.html#introduction). 

The key functions in the `cobalt` package are: 

* `bal.tab()` for tables of balance statistics 

* `bal.plot()` for plots comparing the distribution of a variable before and after matching
* `love.plot()` for a single plot comparing balance before and after matching for all variables `r emoji::emoji('heart')`

Each of these functions take a `matchit` object as the main argument (`cobalt` also works with other packages that undertake matching). Let's check out some examples:


#### `bal.tab()`

The `bal.tab()` function produces a table that summarises the balance before and after matching for all variables included in matching. Different balance metrics are available with the default being the mean difference. 

Setting `un = TRUE` includes the unstandardised data and setting `binary` and `continuous` to `raw` shows the raw mean differences. Standardised mean differences can be presented by setting `binary` and `continuous` to `std`.

```{r ex7, exercise = TRUE, exercise.lines = 8}
match4 <- matchit(treat ~ age + educ + race + married + nodegree + re74 + re75, data=lalonde, method = "nearest")

bal.tab(match4, un = TRUE, binary = "raw", continuous = 'raw')

```

In the default code above we can see, for example, the mean age difference between treated and untreated individuals was -2.2 years whereas in the mathced data the mean difference in age was reduced to 0.5 years. 

Try changing the the `binary` and `continuous` arguments to `std`. What changes and why would this be useful?

*** 

#### `bal.plot()`

THe `bal.plot()` function compares the distribution of a covariate before and after matching. You could use this to assess the degree of common support. Let's look at an example.
...
Here we can see that treated individuals have a narrow age range compared to untreated individuals. But at least we know there should be suitable matches in terms of age for our treated individuals. Try editing the code above to examine some of the other variables. 


#### `love.plot()` `r emoji::emoji('heart')`

A love plot is a popular plot that compares the balance before and after matching for all covariates included in the matching procedure. 
...
There are many options that will allow you to refine this plot, enter `?love.plot` at the console for more details. 
...
Examine the balance diagnostics using the `cobalt` functions. Which approach achieves the best balance? 
...
### Activity 1. Model dependency
Play with the toy example illustrating how matching removes model dependency using this [interactive Shiny app](https://cbdrh.shinyapps.io/matching-model-dependency/). The app can also be launched by entering `hdat9700tutorials::run('matching-1')` at the console. 

This app emulates the example discussed by Professor Gary King in this weeks pre-reading, check it out below:

[![](images/prof-king-vid.png){width='25%'}](https://youtu.be/rBv39pK1iEs?t=199)


*** 

### Activity 2. The MatchIt Package
Explore some of the functionality of the MatchIt package using this [interactive Shiny app](https://cbdrh.shinyapps.io/matching-2). The app can also be launched by entering `hdat9700tutorials::run('matching-2')` at the console. 
...
The key message from this chapter is that it is possible to identify estimates of causal effects from observational data, provided the three identifiability assumptions are met. Re call these three assumptions are 
