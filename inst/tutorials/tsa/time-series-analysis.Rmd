...
set.seed(123)
x1 <- sample(40:80, 48, replace=TRUE) %>% ts(start=c(2005,7), end=c(2009,6), frequency=12)
...
Each section will be followed by a practical session where you will write your own R code to explore the concepts and methods we discuss.   

The prereadings for this chapter are:

* "Time series regression studies in environmental epidemiology" by 
Krishnan Bhaskaran, Antonio Gasparrini, Shakoor Hajat, Liam Smeeth, and Ben Armstrong available [here](https://academic.oup.com/ije/article/42/4/1187/657875) or on OpenLearning. This paper focusses on environmental epidemiology but many of the concepts apply to other types of time series data too.

* "On Time Series Analysis of Public Health and Biomedical Data" by Scott L Zeger, Rafael Irizarry and Roger D Peng available [here](https://www.annualreviews.org/doi/10.1146/annurev.publhealth.26.021304.144517) or on OpenLearning.
...
While the following two papers are about intervention analysis (covered in the next Chapter), they also start by discussing properties of time series data which is relevant for this Chapter:
...
To interpret time series data, it is important to understand the underlying structure, and how this affects its behaviour. For instance, if you observe a sudden drop in prescriptions for antibiotics in a given month, how do you know if it represents a true decline, or is just be part of the normal variation of those data?
...
![**Figure.** An example of a time series: smallpox, polio and measles before and after introduction of vaccination (Source: Our World in Data https://ourworldindata.org/)](images/Vaccination.jpg)
...
# Let's assume the vector represents monthly data from July 2005 to June 2009.
x1 <- ts(vector1, start=c(2005,7), end=c(2009,6), frequency=12)

# Now plot the time series. ts objects are automatically be plotted as line graphs.
plot(x1)
...
plot(mort, plot.type="m", xlab="Week", col="blue", lty="solid", main="Fig 1: LA Respiratory and Cardiovascular Mortality, 1970-1979")
...
2. **Seasonality:** Seasonality occurs when the time series is affected by calendar time, such as time of year or day of the week. Seasonality is always of a fixed and known frequency. It can be due to natural causes, such as the variation in weather patterns between seasons, or business/administrative processes, such as differences between weekends and weekdays. Whether there is seasonality will depend on the time unit of your series (e.g. daily, weekly, etc.).

3. **Error:** The error is the remaining random variation unexplained by the trend or seasonality. Extreme values that are much larger or smaller than expected are considered outliers.
...
Load the `flu` dataset which is part of the `astsa` package. It contains data on pneumonia and influenza deaths per 10,000 population in the United States. It is already a ts object and does not have to be converted.
...
Now visualise the flu time series by plotting it. Think about the kind of pattern that you expect to see. Always remember when creating graphs to choose clear axis labels, and to set the y-axis limits to something reasonable, as axes that are too narrow or too wide can be misleading or difficult to interpret.
...
legend("topleft", legend=c("Observed","Seasonally adjusted"), col=c("blue","red"), lty=c("solid","dashed"),cex=.8)
```
...
legend("topleft", legend=c("Observed","Trend"), col=c("blue","red"), lty=c("solid","dashed"),cex=.8)
```

***

## Practical 1: Visualising time series data ##
...
```{r exercise1_1,exercise=TRUE, tidy=TRUE, exercise.eval=FALSE, exercise.lines=9}
...
```{r exercise1_2,exercise=TRUE, tidy=TRUE, exercise.eval=FALSE, exercise.lines=14}
...
```{r exercise1_3,exercise=TRUE, tidy=TRUE, exercise.lines=7, exercise.eval=FALSE, exercise.lines=6}
...
```{r exercise1_4, exercise=TRUE,tidy=TRUE, exercise.eval=FALSE, fig.height=4, fig.width=5, exercise.lines=10}
...
# To plot on the same facet, use the argument plot.type="s" 
# To plot on the different facets, use the argument plot.type="m"
```
```{r exercise1_4-hint-2}
# To modify the plot, use the following code:
plot(resp_ts, plot.type=, col= , lty= , ylab= , xlab= , ylim= )
```
```{r exercise1_4-hint-3}
# Too add a legend use:
legend()
```
***

#### Exercise 1.5 Data decomposition
```{r exercise1_5, exercise=TRUE,tidy=TRUE, exercise.eval=FALSE, fig.height=4.5, fig.width=5, exercise.lines=12}
# Save the components of the decomposed series for males into a separate data frame (call it "males").


# When you plotted your series in Exercise 1.4, did you observe any trend? any seasonality? any outliers?
# Create a plot of the decomposed time series for males and see if it confirms what you observed above.



# Plot the trend on the *same* plot as the raw data for males only. Remember to use colours and a legend to distinguish the series.
...
# To plot the decomposed series:
plot(males)
```
```{r exercise1_5-hint-3}
# To combine the raw data with the trend data which can then be plotted, use: 
...
```{r exercise1_6,exercise.lines=6, tidy=TRUE, exercise=TRUE, exercise.eval=FALSE}
...
One way of dealing with non-stationarity is to apply a transformation to the data. If the variance is changing over time, this can be addressed by a natural log transformation ($\log(Y_t)$).

On the other hand, if there is a linear trend in the data, taking the first difference of the data ($Y_t-Y_{t-1}$) will usually result in a stationary series. If there is still a trend, then a second differencing will be required, but that is not common. 
...
Now the series is almost stationary, but seasonality is still a problem - we will deal with this later! 
...
quiz(caption= "Quiz 2_1: Stationarity",
    ...
    answer("All of them"),
    random_answer_order = FALSE, allow_retry = TRUE),
    question("Which is not a property of a weakly stationary series?",
...
## Statistical properties: Autocorrelation

As mentioned, time series data cannot usually be analysed using standard regression methods due to correlation between observations within the series, i.e. autocorrelation. The presence of autocorrelation can lead to incorrect standard errors.
...
## Statistical properties: Seasonality 
...
We already saw how to check for seasonality using visualisation. However, ACF plots can also help identify seasonality. If you have monthly seasonality, then you would expect to see significant autocorrelation at lag 12. If you don't, then the seasonality may not be strong enough to need adjustment.
...
## Practical 2: Statistical properties of time series
...
```{r exercise2_1,tidy=TRUE, exercise=TRUE, exercise.eval=FALSE, exercise.lines=23}
...
# Is there a trend? If so, take a first order difference and plot the data again



# Is there seasonality? If so, take a seasonal difference and plot the data again
...
# If there is seasonality transform the data using diff(, lag=12)
plot(diff(diff(log(air)),12))
```
```{r exercise2_1-hint-4}
...
```{r exercise2_2,exercise=TRUE, tidy=TRUE, exercise.eval=FALSE, exercise.lines=12}
# Plot the ACF/PACF plots on the raw data - what does this tell you about autocorrelation and seasonality?



# Plot the ACF/PACF plots on the transformed data from above



# What do these tell you about your data?
...
```{r exercise2_3,exercise=TRUE, tidy=TRUE, exercise.eval=FALSE, exercise.lines=7}
...
The most basic type is **ordinary least squares (OLS) regression models** (often linear regression) that use time as a predictor variable. 
...
In real-life research and practice the patterns of the data can be complex, with considerable error, and it can be difficult to determine the patterns in the data. After creating your model, it is also important to check for autocorrelation of residuals - if present, even after controlling for trend and seasonality, an OLS regression model may not be appropriate.

There are various approaches for modelling data with autocorrelation and/or complex seasonality. We will cover one such approach, **ARIMA models**, which relate the present value of a series to past values and past errors. 

***

## Autoregressive, moving average and seasonal models
...
quiz(caption = "Quiz 3.1: Basic models", 
    ...
    answer("White noise processes can have seasonality", correct=TRUE, message="White noise processes have no trend, seasonality or autocorrelation"),
...
The first step in fitting your ARIMA model is to plot your time series to check for non-stationarity, i.e. changing variance or trends, as well as seasonality. This may include a plot of the time series, a plot of the decomposed series, seasonal plots, and/or ACF plots.

To deal with non-constant variance, apply a log transformation. Take note if a first difference (to adjust for trends) or seasonal difference (to adjust for seasonality) may be required - you do not have the apply the differencing yourself as this will be taken care of within the ARIMA model. Generally, if your series has a trend and a first difference (*d*=1) is required, if the data are seasonal, a seasonal difference (*D*=1) will also be required. It is unusual to require more than one first/seasonal difference.
...
Next estimate your model, and check the model fit by plotting the residuals. Recall that there are three assumptions your residuals must meet: normally distributed, constant variance, and independence (no autocorrelation). If your model has a poor fit, then you will have to choose a different AR or MA order. Sometimes, determining the AR and MA terms is an iterative process, involving trial and error. 

Aim for the simplest model (i.e. lowest AR and MA orders), that has a good fit. Information criteria (AIC and BIC) can be useful in determining the best fitting model.
...
Now it gives us ARIMA(2,0,0). We also see that AR1 = 1.44, and AR2 = -0.67, not that different from the true parameters (1.5 and -0.75). Since ARIMA(2,0,0) is a simpler model (fewer parameters) than ARIMA(3,0,1), it is preferable.
...
```{r exercise3_1,tidy=TRUE, exercise=TRUE, exercise.eval=FALSE,  exercise.lines=6}
...
```{r exercise3_2,tidy=TRUE, exercise=TRUE, exercise.lines=7, exercise.eval=FALSE }
...
```{r exercise3_3,tidy=TRUE, exercise=TRUE, exercise.eval=FALSE , fig.height=7, fig.width=5, exercise.lines=9}
