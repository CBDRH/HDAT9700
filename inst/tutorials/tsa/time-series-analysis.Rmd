...
The reference text for this chapter is "Time series analysis and its applications: with R examples" (Chapters 1-3) by Robert H Shumway and David S Stoffer, can be downloaded [here](https://www.stat.pitt.edu/stoffer/tsa4/) or through the UNSW library.  David S Stoffer is also the author of the `astsa` R package (**A**pplied **S**tatistical **T**ime **S**eries **A**nalysis).
...
![**Figure 1.1.** An example of a time series: all cause mortality during the 1918 influenza pandemic (Source: Public domain)](images/Influenza_Pandemic.png)
...
The table below includes guidance on selecting the most appropriate AR and MA terms. In practice, there are automated algorithms in R that will select the most appropriate orders for you based on statistial criteria, which we will see later.
