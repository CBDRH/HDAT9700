...
ui <- dashboardPage(
    ...
    # Application title
    dashboardHeader(title = "Matching"),
    ...
    # Sidebar with a slider input for number of bins
        ...
        dashboardSidebar(width = 350,
            ...
            p(HTML(paste0(
                ...
                tags$a(href="https://www.youtube.com/watch?v=rBv39pK1iEs", target="_blank", "here"), " (see from about 3:20 - 8:00 mins)."))),
            hr(),
            h3("Aims"),
            p("Using the applet, demonstrate the following three points:"),
            tags$ol(
                ...
                tags$li("Over repeated samples, the matched estimate are unbiased while the unmatched estimates are biased.")
            ),
            hr(),
            h3("Controls"),
            actionButton("button", "Run", width = '50%', icon = icon('flag-checkered')),
            helpText("Click run to generate a new set of simulated data"),
...
server <- function(input, output) {
    ...
    # Find matches
    match1 <- matchit(tx ~ x1, data=d0, method = "nearest", distance = "logit")
...
observeEvent(input$button, {
    ...
    output$est <- renderPlot({
        ...
        data$est %>% tidyr::gather() %>%
            ...
            geom_dotplot(dotsize = 1, binwidth = 0.05) +
            scale_color_manual(values=c("#D81B60", "#F012BE", "#3D9970", "#01FF70")) +
            scale_fill_manual(values=c("#D81B60", "#F012BE", "#3D9970", "#01FF70")) +
            scale_x_continuous(name = "Point estimate", limits = c(1, 3), breaks = seq(1, 3, 0.5), minor_breaks = NULL) +
    ...
    output$estBoxq0 <- renderUI({
        shinydashboard::infoBox(
            title = "Estimated treatment effect",
            subtitle = "All data | Quadratic fit",
    ...
    output$estBoxl1 <- renderUI({
        shinydashboard::infoBox(
            title = "Estimated treatment effect",
            subtitle = "Matched data | Linear fit",
    ...
    output$estBoxq1 <- renderUI({
        shinydashboard::infoBox(
            title = "Estimated treatment effect",
            subtitle = "Matched data | Quadratic fit",
